...
#define SET_SCL(config, pinstate) \
        do{\
            GPIO_WriteBit(config->scl.port, config->scl.pin, pinstate);\
        }while(0)

#define SET_SDA(config, pinstate) \
        do{\
            GPIO_WriteBit(config->sda.port, config->sda.pin, pinstate);\
        }while(0)
...
static void __pvt_on_init_iic_gpio(OLED_SOFT_IIC_Private_Config* config)
{
    ...
    GPIO_InitStructure.GPIO_Pin = config->scl.pin;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(config->scl.port, &GPIO_InitStructure);
    GPIO_InitStructure.GPIO_Pin =  config->sda.pin;
    GPIO_Init(config->sda.port, &GPIO_InitStructure);
    SET_SCL(config, 1);
    SET_SDA(config, 1);
}
