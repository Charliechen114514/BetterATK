...
CCGPIOType  adc_gpio;
static void __adc_post_init()
{
    configure_ccgpio(&adc_gpio, &adc_init);
}

CCADC_Handle    adc_handle;
void init_adc()
{
    ADC_InitTypeDef init;
    init.ClockPrescaler=ADC_CLOCK_SYNC_PCLK_DIV4;   //4分频，ADCCLK=PCLK2/4=90/4=22.5MHZ
    init.Resolution=ADC_RESOLUTION_12B;             //12位模式
    init.DataAlign=ADC_DATAALIGN_RIGHT;             //右对齐
    init.ScanConvMode=DISABLE;                      //非扫描模式
    init.EOCSelection=DISABLE;                      //关闭EOC中断
    init.ContinuousConvMode=DISABLE;                //关闭连续转换
    init.NbrOfConversion=1;                         //1个转换在规则序列中 也就是只转换规则序列1 
    init.DiscontinuousConvMode=DISABLE;             //禁止不连续采样模式
    init.NbrOfDiscConversion=0;                     //不连续采样通道数为0
    init.ExternalTrigConv=ADC_SOFTWARE_START;       //软件触发
    init.ExternalTrigConvEdge=ADC_EXTERNALTRIGCONVEDGE_NONE;//使用软件触发
    init.DMAContinuousRequests=DISABLE;             //关闭DMA请求
    init_adc_handle(&adc_handle, ADC1, &init, ADC_CHANNEL_5, __adc_post_init);
}
