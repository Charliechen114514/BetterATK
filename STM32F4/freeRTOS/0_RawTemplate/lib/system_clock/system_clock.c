...
#include "hal_system_level_callback.h"
/*
...
static inline void __clk_init_die()
{
    while (1)
        ;
}

static TIM_TypeDef *tim_using_as_src;
...
uint32_t provide_application_sysfreq(uint32_t us)
{
...
#else
    uint32_t start = tim_using_as_src->CNT;
    while ((uint16_t)(tim_using_as_src->CNT - start) < us)
        ;
#endif
}
