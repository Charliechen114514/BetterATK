...
{
    device_size_t max_y = max_uint16(line->p_left.y, line->p_right.y);
    device_size_t min_y = min_uint16(line->p_left.y, line->p_right.y);
    CCGraphic_Point p;
    p.x = line->p_left.x;
    for(device_size_t i = min_y; i <= max_y; i++)
    {
        p.y = i;
...
{
    device_size_t max_x = max_uint16(line->p_left.x, line->p_right.x);
    device_size_t min_x = min_uint16(line->p_left.x, line->p_right.x);
    CCGraphic_Point p;
    p.y = line->p_left.y;
    for(device_size_t i = min_x; i <= max_x; i++)
    {
        p.x = i;
        CCGraphic_draw_point(handler, &p);
    }
...
void __pvt_BresenhamMethod_line(CCDevice_t* handler, CCGraphic_Line* line)
{
